defmodule Tabebuia.IntegrationTest do
  use ExUnit.Case, async: false  # Not async because of file system operations
  ...
  describe "extract/2" do
    test "extracts archive to directory", %{tmp_dir: tmp_dir, archive_path: archive_path} do
      ...
      assert File.exists?(Path.join(extract_dir, "subdir/file3.txt"))
      
      # Verify content
      assert File.read!(Path.join(extract_dir, "file1.txt")) == "Content 1"
    end

    test "creates destination directory if it doesn't exist", %{tmp_dir: tmp_dir, archive_path: archive_path} do
  ...
  describe "round-trip: create → extract" do
    ...
    test "preserves directory structure", %{tmp_dir: tmp_dir, archive_path: archive_path} do
      # Create deep directory structure
      deep_dir = Path.join(tmp_dir, "a/b/c/d")
      File.mkdir_p!(deep_dir)
      File.write!(Path.join(deep_dir, "deep.txt"), "Deep content")
      
      # Create archive
      :ok = Tabebuia.create(archive_path, [Path.join(tmp_dir, "a")])
      ...
      # Verify structure
      assert File.exists?(Path.join(extract_dir, "a/b/c/d/deep.txt"))
    end
  end
end
